...
const TEMP_DIR = join(ROOT, "temp_downloads");
...
async function compositeLabel(srcPath, destPath, productName, dosageStrength, category) {
  const stripeColor = COLOR_MAP[category] || "#C5A059";
...
<svg width="600" height="600" xmlns="http://www.w3.org/2000/svg">
  ...
  <rect x="510" y="490" width="36" height="18" rx="4" fill="#0A1F3D" />
  <text x="528" y="502" font-family="Arial, sans-serif" font-size="9" font-weight="800" fill="#C5A059" text-anchor="middle">Rx</text>
  <rect x="20" y="574" width="560" height="6" rx="2" fill="${stripeColor}" />
</svg>
`;
...
async function main() {
  ...
  console.log(`Configuring search queries for ${products.length} products...`);
  ...
  for (const p of products) {
    ...
    // Pick queries based on container types for better search relevance
    const containerType = getContainerType(p.category, p.slug, dosageForm);
    let primaryQuery = "";
    let altQuery = "";
    
    if (containerType === "vial") {
      primaryQuery = `${productName} injection vial medicine`;
      altQuery = `${productName} medical syringe packaging`;
    } else if (containerType === "cream") {
      primaryQuery = `${productName} topical cream tube medicine`;
      altQuery = `${productName} skin gel packaging`;
    } else if (containerType === "dropper") {
      primaryQuery = `${productName} hair serum bottle dropper`;
      altQuery = `${productName} topical treatment packaging`;
    } else if (containerType === "bottle") {
      primaryQuery = `${productName} pills bottle medicine`;
      altQuery = `${productName} tablets capsules packaging`;
    } else {
      primaryQuery = `${productName} medical packaging box`;
      altQuery = `${productName} clinical supplies kit`;
    }
  ...
  const queriesPath = join(TEMP_DIR, "queries.json");
  writeFileSync(queriesPath, JSON.stringify(scrapeQueries, null, 2));
  console.log(`Saved queries to ${queriesPath}`);

  // 4. Run Python Scraper
  console.log("Starting Python image scraper with DuckDuckGo Search...");
  const pythonScript = "C:\\Users\\donfi\\.gemini\\antigravity\\brain\\207892ed-ca33-4bc8-ac8d-0b3c8845796d\\scratch\\ddg_image_scraper.py";
  
  let pythonCommand = `"C:\\Users\\donfi\\scoop\\apps\\python\\current\\python.exe" -u "${pythonScript}" "${queriesPath}" "${TEMP_DIR}"`;
  
  try {
    execSync(pythonCommand, { stdio: "inherit" });
  } catch (err) {
    console.warn("Python execution failed or interrupted. Proceeding with processing downloaded files or falling back to SVG mockups.");
  }

  // 5. Read scraper results and process each product
  console.log("\nProcessing images and compositing labels...");
  let countScraped = 0;
  let countSvg = 0;

  for (const p of processedList) {
    const primaryDest = join(PRODUCTS, `${p.slug}.jpg`);
    const altDest = join(PRODUCTS, `${p.slug}-alt.jpg`);
    
    const scrapedPrimaryPath = join(TEMP_DIR, `${p.slug}.jpg`);
    const scrapedAltPath = join(TEMP_DIR, `${p.slug}-alt.jpg`);

    // --- Process Primary Image ---
    if (existsSync(scrapedPrimaryPath)) {
      console.log(`[${p.slug}] Processing scraped primary image...`);
      try {
        await compositeLabel(scrapedPrimaryPath, primaryDest, p.productName, p.dosageStrength, p.category);
        countScraped++;
      } catch (err) {
        console.warn(`  Failed processing primary for ${p.slug}: ${err.message}. Generating SVG instead.`);
        const svg = generateSvgMockup(p.productName, p.dosageStrength, p.dosageForm, p.category, p.slug, false);
        await sharp(Buffer.from(svg)).jpeg({ quality: 90 }).toFile(primaryDest);
        countSvg++;
      }
    } else {
      console.log(`[${p.slug}] Generating premium SVG mockup for primary...`);
      const svg = generateSvgMockup(p.productName, p.dosageStrength, p.dosageForm, p.category, p.slug, false);
      await sharp(Buffer.from(svg)).jpeg({ quality: 90 }).toFile(primaryDest);
      countSvg++;
    }
    ...
    // --- Process Alt Image ---
    if (existsSync(scrapedAltPath)) {
      console.log(`[${p.slug}] Processing scraped alt image...`);
      try {
        await compositeLabel(scrapedAltPath, altDest, p.productName, p.dosageStrength, p.category);
        countScraped++;
      } catch (err) {
        console.warn(`  Failed processing alt for ${p.slug}: ${err.message}. Generating SVG instead.`);
        const svg = generateSvgMockup(p.productName, p.dosageStrength, p.dosageForm, p.category, p.slug, true);
        await sharp(Buffer.from(svg)).jpeg({ quality: 90 }).toFile(altDest);
        countSvg++;
      }
    } else {
      console.log(`[${p.slug}] Generating premium SVG mockup for alt...`);
      const svg = generateSvgMockup(p.productName, p.dosageStrength, p.dosageForm, p.category, p.slug, true);
      await sharp(Buffer.from(svg)).jpeg({ quality: 90 }).toFile(altDest);
      countSvg++;
    }
  ...
  console.log(`\nFinished! Successfully processed all product assets:`);
  console.log(`- Scraped images: ${countScraped}`);
  console.log(`- Custom SVG renders: ${countSvg}`);
  console.log(`Total output: ${products.length * 2} unique assets saved to public/images/products/`);
}
